...
public class DateUtil {
	...
	/**
	 * DateUtil.getTimeBefore("2017-03-21 14:00", "2017-03-21 14:30", 10)
	 * 得到一个时间段 分钟 的所有时间点 集合
	 * @param startTime 开始时间
	 * @param endTime   终止时间
	 * @param minute 分钟
	 * @return 返回一个时间 yyyy-MM-dd HH:mm  数组
	 */
	public static List<String> getTimeBefore(String startTime, String endTime, int minute){
		...
		try {
			...
			interval = (int) ((eTime.getTime() - sTime.getTime()) / (1000));// 时间差 单位秒
			size = interval/(minute*60);
			for (int i=0;i<size;i++){
				long l = sTime.getTime()+(long)(i*minute*1000*60);
