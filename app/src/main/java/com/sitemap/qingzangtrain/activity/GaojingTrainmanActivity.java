...
import com.sitemap.qingzangtrain.adapter.GaoJingTrainmanAdapter;
 ...
 */
@ContentView(R.layout.activity_gaojing_trainman)
public class GaojingTrainmanActivity extends BaseActivity implements View.OnClickListener ,GaoJingTrainmanAdapter.OnCallBackLook,PullToRefreshLayout.OnRefreshListener{
    private GaojingTrainmanActivity mContext;//本类
    ...
    private GaoJingTrainmanAdapter adapter;//适配器
...
//            initListView(mList);
    ...
    private void initListView(List<TroublesModel> list){
        adapter = new GaoJingTrainmanAdapter(mContext,list,this);
...
//            mRefresh.loadmoreFinish(PullToRefreshLayout.FAIL);
     ...
     * 更改核查状态
     */
    private void uploadCheck(String userID,String troubleID){
        if (MyApplication.getNetObject().isNetConnected()) {
            progressDialog = RoundProgressDialog.createDialog(this);
            if (progressDialog != null && !progressDialog.isShowing()) {
                progressDialog.setMessage("加载中...");
            ...
            http.sendGet(RequestCode.UPLOADCHECK, WebUrlConfig.updateCheck(userID, troubleID));
        } else {
            ToastUtil.showBottomShort(this, RequestCode.NOLOGIN);
        }
    }
    ...
    public void onLookInfo(int pos) {
        currPos = pos;
        uploadCheck(MyApplication.loginModel.getUserID(),mList.get(pos).getTroubleID());
    }
